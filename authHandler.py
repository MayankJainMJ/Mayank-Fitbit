...
def GetNewAccessToken(RefToken):
	...
	tokenreq.add_header('Authorization', 'Basic ' + (base64.b64encode((client_id + ":" + client_secret).encode())).decode())
	...
	try:
		...
		FullResponse = (tokenresponse.read()).decode()
		...
		print_json('error', err.code, json.loads((err.read()).decode()))
		sys.exit(1)
...
def MakeAPICall(InURL,AccToken,RefToken):
	...
	#Catch errors, e.g. A 401 error that signifies the need for a new access token
		...
		HTTPErrorMessage = (err.read()).decode()
